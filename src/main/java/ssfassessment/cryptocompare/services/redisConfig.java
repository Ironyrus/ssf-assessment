...
public class redisConfig {
    ...
    public RedisTemplate<String, artObj> redisTemplate() {
        ...
        Jackson2JsonRedisSerializer jackson2JsonJsonSerializer = new Jackson2JsonRedisSerializer(artObj.class);

        final JedisClientConfiguration jedisClient = JedisClientConfiguration.builder().build();
        final JedisConnectionFactory jedisFac = new JedisConnectionFactory(config, jedisClient);
        jedisFac.afterPropertiesSet();
        RedisTemplate<String, artObj> template = new RedisTemplate<String, artObj>();
